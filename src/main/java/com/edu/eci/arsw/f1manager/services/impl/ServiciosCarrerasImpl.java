...
public class ServiciosCarrerasImpl implements ServiciosCarreras,CommandLineRunner{
     ...
     */
    @Override
    public void actualizarJugador(String usuario, int puntos, int posicion, float tiempo, int automovil, int estrategia, String idCarrera, String equipo) throws ExcepcionServiciosCarreras{
        try{
            jugador.save(new Jugador(usuario, jugador.findByUsuario(usuario).getContrasena(), jugador.findByUsuario(usuario).getCorreo(), jugador.findByUsuario(usuario).getFechaNacimiento(), jugador.findByUsuario(usuario).getGenero(), equipo, puntos, posicion, tiempo, automovil, estrategia, idCarrera));
        }catch(PersistenceException e){
            throw new ExcepcionServiciosCarreras(e.getMessage());
        }catch(Exception e){
    ...
    public void crearJugador(String usuario, String contrasena, String correo, String fechaNacimiento, String genero, int puntos, int posicion, float tiempo, int automovil, int idEstrategia, String idCarrera) throws ExcepcionServiciosCarreras{
        if (jugador.findByUsuario(usuario) == null){
            ...
            jugador.save(new Jugador(usuario, contrasena, correo, fechaNacimiento,"img/team1.jpg", genero, puntos, posicion, tiempo, automovil, idEstrategia, idCarrera));
        }else{
